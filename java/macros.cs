...
<?cs # A link to a type, or not if it's a primitive type
        link: whether to create a link at the top level, always creates links in
              ...
              Overloaded version to support use of 'nav' parameter, which when
              true, will not include the generics in the class name (good for
              sidenav lists)
...
def:tag_list(tags) ?><?cs
  each:tag = tags ?><?cs
      ...
      elif:tag.kind == "@see" ?><code><a href="<?cs if:string.length(type.federated) == 0 || string.find(tag.href, type.federated) == -1 ?><?cs var:toroot ?><?cs /if ?><?cs var:tag.href ?>"><?cs var:tag.label ?></a></code><?cs
      elif:tag.kind == "@seeHref" ?><a href="<?cs var:tag.href ?>"><?cs var:tag.label ?></a><?cs
      elif:tag.kind == "@seeJustLabel" ?><?cs var:tag.label ?><?cs
      elif:tag.kind == "@value" ?><code><a href="<?cs if:string.length(type.federated) == 0 || string.find(tag.href, type.federated) == -1 ?><?cs var:toroot ?><?cs /if ?>"><?cs var:tag.text ?></a></code><?cs
...
def:short_descr(obj) ?><?cs
  if:subcount(obj.deprecated) ?>
      <em>This <?cs var:obj.kind ?> is deprecated.
...
def:deprecated_warning(obj) ?><?cs
  ...
  <p class="caution">
      <strong>This <?cs var:obj.kind ?> is deprecated.</strong><br/> <?cs
